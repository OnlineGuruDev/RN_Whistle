...
import { View, StyleSheet, SectionList, Text, NativeModules } from 'react-native'
...
class PushList extends React.Component {
  ...
  showMFMessage = async (id, included) => {
    try {
  ...
  renderItem = ({item}) => {
    ...
    return(
      <Touchable onPress={ () => this.showMFMessage(item.id, included)}>
...
export default connect(mapStateToProps)(PushList)
